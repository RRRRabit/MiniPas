...
static std::string toTargetCode(const Quadruple& q) {
    if (q.op == ":=") return "MOV " + q.result + ", " + q.arg1;
    if (q.op == "+") return "ADD " + q.result + ", " + q.arg1 + ", " + q.arg2;
    if (q.op == "-") return "SUB " + q.result + ", " + q.arg1 + ", " + q.arg2;
    if (q.op == "*") return "MUL " + q.result + ", " + q.arg1 + ", " + q.arg2;
    if (q.op == "/") return "DIV " + q.result + ", " + q.arg1 + ", " + q.arg2;
    if (q.op == "if" || q.op == "do") return "FJ " + q.arg1 + ", Lfalse";
    if (q.op == "el" || q.op == "we") return "JMP Lnext";
    if (q.op == "param") return "ARG " + q.arg1;
    if (q.op == "call") return "CALL " + q.arg1 + ", " + q.arg2;
    if (q.op == "ret") return "RET " + q.arg1;
    return "MARK " + q.op;
}
