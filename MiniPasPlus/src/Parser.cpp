...
void Parser::parseTypeDeclPart()
{
    ...
    while (check(TokenType::Identifier))
    {
        ...
        else if (check(TokenType::Keyword, "array"))
        { // 检查当前 Token 类型或文本
            parseArrayTypeDecl(typeName.lexeme);
        }
        else
        {
            throw runtime_error("type 声明当前支持 record 和 array");
        }
...
void Parser::parseRecordTypeDecl(const string &typeName)
{
...
void Parser::parseArrayTypeDecl(const string &typeName)
{
    consume(TokenType::Keyword, "array", "array 类型缺少 array");
    consume(TokenType::Delimiter, "[", "array 后缺少 [");
    int low = stoi(consume(TokenType::Constant, "", "数组下界应为常量").lexeme);
    consume(TokenType::Delimiter, "..", "数组上下界之间缺少 ..");
    int high = stoi(consume(TokenType::Constant, "", "数组上界应为常量").lexeme);
    consume(TokenType::Delimiter, "]", "数组后缺少 ]");
    consume(TokenType::Keyword, "of", "array 缺少 of");
    string elementType = parseTypeName();
...
// 解析函数参数
void Parser::parseParamList(const string &functionName)
{
    while (true)
    {
...
void Parser::parseVarDeclPart(const string &scope)
{
    // 解析 var 声明区。
    // scope 为空：全局变量；
    // scope 非空：函数局部变量。
    ...
    while (check(TokenType::Identifier))
    {
        ...
        for (const auto &name : names)
        {
            string kind = scope.empty() ? "variable" : "local variable of " + scope;
...
void Parser::parseAssignStmt()
{
    ...
    {
        string baseName = left.substr(0, left.find('.'));
        string fieldName = left.substr(left.find('.') + 1);
    ...
    {
        throw runtime_error("赋值类型不兼容: " + leftType + " := " + right.type);
    }
...
Parser::ExprResult Parser::parseCondition()
{
    ...
    string op = consume(TokenType::Operator, "", "条件缺少关系运算符").lexeme;
    ExprResult right = parseExpression();
...
Parser::ExprResult Parser::parseExpression()
{
    ...
    while (check(TokenType::Operator, "+") || check(TokenType::Operator, "-"))
    {
        ...
        string temp = newTemp(left.type == "real" || right.type == "real" ? "real" : "integer");
        emit(op, left.place, right.place, temp); // 生成四元式
        left = {temp, "integer"};
    }
    return left;
...
Parser::ExprResult Parser::parseFactor()
{
    if (check(TokenType::Constant))
    {
        ...
        return {c.lexeme, c.lexeme.find('.') == string::npos ? "integer" : "real"};
    }
    ...
    if (check(TokenType::Identifier))
    {
        ...
        string place = name.lexeme;
        string type = requireVariableDeclared(name.lexeme).typeName;
...
Parser::ExprResult Parser::parseFunctionCall(const Token &functionName)
{
    ...
    {
        throw runtime_error(functionName.lexeme + " 不是函数，不能调用");
    }
    ...
    string result = newTemp(functionSymbol.typeName);
    emit("call", functionName.lexeme, to_string(args.size()), result);
...
string Parser::parseTypeName()
{
    if (check(TokenType::Keyword, "integer") || check(TokenType::Keyword, "real") ||
        check(TokenType::Keyword, "char") || check(TokenType::Keyword, "boolean"))
    { // 检查当前 Token 类型或文本
...
// 生成新的临时变量名并加入符号表。
string Parser::newTemp(const string &type)
{
    string name = "t" + to_string(++tempIndex_);
    symbolTable_.add({name, type, "temporary", 0, 0}); // 写入符号表
    return name;
}

// 生成一条四元式并写入中间代码序列。
void Parser::emit(const string &op, const string &a, const string &b, const string &r)
{
...
void Parser::buildBasicBlocks()
{
    ...
    {
        const string &op = result_.quadruples[i].op;
...
// 要求标识符已声明，否则抛出语义错误。
const Symbol &Parser::requireVariableDeclared(const string &name) const
{
    const Symbol *symbol = symbolTable_.find(name);
    if (symbol == nullptr)
    {
        throw runtime_error("标识符未声明: " + name);
    }
    return *symbol;
}

// 赋值兼容性判断（仅支持同类型或 integer 到 real）。
bool Parser::canAssign(const string &targetType, const string &sourceType) const
{
...
// 检查数组下标类型并返回元素类型。
string Parser::checkArrayIndexAndGetElementType(const string &arrayName, const ExprResult &index) const
{
    if (index.type != "integer")
    {
        throw runtime_error("数组下标必须是 integer: " + arrayName);
    }
    ...
    if (arrayType == nullptr)
    {
        throw runtime_error(arrayName + " 不是数组，不能使用 []");
    }
...
// 检查记录字段合法性并返回字段类型。
string Parser::checkRecordFieldAndGetType(const string &recordVarName, const string &fieldName) const
{
    ...
    if (field == nullptr)
    {
        throw runtime_error("record 字段不存在: " + recordVarName + "." + fieldName);
    }
...
// 找到某函数全部形参信息。
vector<ParameterInfo> Parser::findFunctionParams(const string &functionName) const
{
...
// 校验实参与形参数量和类型匹配性。
void Parser::checkArgumentList(const string &functionName, const vector<ExprResult> &args) const
{
    vector<ParameterInfo> params = findFunctionParams(functionName);
    if (params.size() != args.size())
    {
        throw runtime_error("函数实参数量不匹配: " + functionName);
    }
    ...
    for (int i = 0; i < static_cast<int>(args.size()); ++i)
    {
        ...
        {
            throw runtime_error("函数参数类型不匹配: " + functionName);
        }
    }
}
